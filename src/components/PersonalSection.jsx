...
import { FaBook, FaLaptopCode, FaPlane, FaPenFancy, FaCoffee } from "react-icons/fa";
...
const colorMap = {
  ...
  FaPenFancy: "bg-pink-100 text-pink-600",
  FaCoffee: "bg-yellow-100 text-yellow-600"
};

// Variants for container with stagger
...
const cardVariants = {
  ...
  visible: { y: 0, opacity: 1, transition: { type: "spring", bounce: 0.2, duration: 0.6 } }
};
...
const PersonalSection = () => (
  <section className="py-16 px-6 sm:px-12 bg-gradient-to-r from-indigo-50 to-blue-50">
    ...
    <motion.div
      className="max-w-5xl mx-auto grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-6"
    ...
    >
      ...
      {/* Coffee & Explore Card */}
      <motion.div
        className="rounded-xl p-6 flex flex-col items-center text-center shadow-md cursor-pointer bg-yellow-100 text-yellow-600 transition-shadow"
        variants={cardVariants}
        whileHover={{ scale: 1.08, rotate: 2, boxShadow: "0 10px 20px rgba(0,0,0,0.2)" }}
      >
        <FaCoffee className="text-4xl mb-3" />
        <h3 className="text-lg font-semibold mb-1">Coffee & Explore</h3>
        <p className="text-gray-700 text-sm">Fuel creativity with a cup of coffee while exploring ideas, tech, and the world.</p>
      </motion.div>
    </motion.div>
